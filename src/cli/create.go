...
func runCreate(cmd *cobra.Command, args []string) {
	...
	dateStr := lib.PromptInput("Date (YYYY-MM-DD, or press Enter for today): ")
	if dateStr == "" {
		dateStr = time.Now().Format("2006-01-02")
	}

	perfectDay, err := models.NewPerfectDay(lib.GenerateID(), title, description, username, dateStr)
	if err != nil {
	...
	for {
		...
		activityDescription := lib.PromptInput("Activity description (optional): ")
		commentary := lib.PromptInput("Personal commentary (optional): ")
		...
		if !lib.PromptConfirm("Add another activity?") {
			break
		}
	}
...
func promptForLocation(placesService *places.PlacesService) *models.Location {
	...
	choice := lib.PromptInput("Choose option (1 or 2): ")
...
func promptForGooglePlace(placesService *places.PlacesService) *models.Location {
	query := lib.PromptInput("Search for place: ")
	if query == "" {
		return nil
	}
	...
	choiceStr := lib.PromptInput("Select place (1-5) or 0 for custom: ")
	...
	area := lib.PromptInput(fmt.Sprintf("Area (suggested: %s): ", placesService.SuggestAreaFromAddress(selectedPlace.Address)))
	if area == "" {
...
func promptForCustomLocation() *models.Location {
	name := lib.PromptInput("Location name: ")
	if name == "" {
		return nil
	}

	area := lib.PromptInput("Area: ")
