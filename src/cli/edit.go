...
func showEditMenu() string {
	...
	return lib.PromptInput("Choose an option (1-5): ")
}
...
func editBasicInfo(perfectDay *models.PerfectDay) {
	...
	newTitle := lib.PromptInput("New title (press Enter to keep current): ")
	if newTitle != "" {
	...
	newDescription := lib.PromptInput("New description (press Enter to keep current): ")
	if newDescription != "" {
	...
	fmt.Printf("Current date: %s\n", perfectDay.Date)
	newDate := lib.PromptInput("New date (YYYY-MM-DD, press Enter to keep current): ")
	if newDate != "" {
...
func showActivitiesMenu(perfectDay *models.PerfectDay) string {
	...
	return lib.PromptInput("Choose an option: ")
}
...
func addNewActivity(perfectDay *models.PerfectDay, placesService *places.PlacesService) {
	...
	activityName := lib.PromptInput("Activity name: ")
	if activityName == "" {
	...
	startTime := lib.PromptInput("Start time (HH:MM): ")
	durationStr := lib.PromptInput("Duration in minutes: ")
	...
	description := lib.PromptInput("Description (optional): ")
	commentary := lib.PromptInput("Commentary (optional): ")
...
func editActivity(perfectDay *models.PerfectDay, placesService *places.PlacesService) {
	...
	indexStr := lib.PromptInput("Enter activity number to edit: ")
	...
	newName := lib.PromptInput("New name (press Enter to keep): ")
	if newName != "" {
		activity.Name = newName
	}

	if lib.PromptConfirm("Edit location?") {
	...
	newStartTime := lib.PromptInput("New start time (HH:MM, press Enter to keep): ")
	if newStartTime != "" {
	...
	newDurationStr := lib.PromptInput("New duration (minutes, press Enter to keep): ")
	if newDurationStr != "" {
	...
	newDescription := lib.PromptInput("New description (press Enter to keep): ")
	if newDescription != "" {
		activity.Description = newDescription
	}

	fmt.Printf("Current commentary: %s\n", activity.Commentary)
	newCommentary := lib.PromptInput("New commentary (press Enter to keep): ")
	if newCommentary != "" {
...
func removeActivity(perfectDay *models.PerfectDay) {
	...
	indexStr := lib.PromptInput("Enter activity number to remove: ")
	index, err := strconv.Atoi(indexStr)
	if err != nil || index < 1 || index > len(perfectDay.Activities) {
		fmt.Println("Invalid activity number.")
		return
	...
	if lib.PromptConfirm("Are you sure?") {
...
func reorderActivities(perfectDay *models.PerfectDay) {
	...
	if lib.PromptConfirm("Sort by time automatically?") {
		perfectDay.SortActivitiesByTime()
...
func saveAndExit(perfectDay *models.PerfectDay, storage *storage.Storage) bool {
	if lib.PromptConfirm("Save changes?") {
		perfectDay.UpdatedAt = time.Now()
